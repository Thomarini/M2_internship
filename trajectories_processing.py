...
if __name__ == "__main__":
    ...
    fps = 25
    #frames, spines = load_frames_spines_from_csv("spines_interpolated_82")
    ...
    plt.plot(spines[:, -1, 0], 850 - spines[:, -1, 1], marker=".", linestyle="dotted", label="Raw head", color = "coral", ms = 16, linewidth=4)
    plt.plot(spines[:, 0, 0], 850 - spines[:, 0, 1], marker=".", linestyle="dotted", label = "Raw tail", color = "limegreen", ms = 16, linewidth=4)
    plt.plot(spines[:, -10, 0], 850 - spines[:, -10, 1], marker=".", linestyle="dotted", label="Raw body", color = "royalblue", ms = 16, linewidth=4)
    ...
    """ plt.plot(filtered_trajectories[:, -1, 0], 850 -filtered_trajectories[:, -1, 1], marker=".", linestyle="dotted", label="Filtered head", color = "red", ms = 16, linewidth=4)
    plt.plot(filtered_trajectories[:, 0, 0], 850 -filtered_trajectories[:, 0, 1], marker=".", linestyle="dotted", label = "Filtered tail", color = "green", ms = 16, linewidth=4)
    plt.plot(filtered_trajectories[:, -10, 0], 850 -filtered_trajectories[:, -10, 1], marker=".", linestyle="dotted", label="Filtered body", color = "blue", ms = 16, linewidth=4) """
    ...
    plt.plot(resampled_trajectories[:, -1, 0], 850 - resampled_trajectories[:, -1, 1], marker=".", linestyle="-", label="Filtered + interpolated head", color = "red", ms = 16, linewidth=4)
    plt.plot(resampled_trajectories[:, 0, 0], 850 - resampled_trajectories[:, 0, 1], marker=".", linestyle="-", label = "Filtered + interpolated tail", color = "green", ms = 16, linewidth=4)
    plt.plot(resampled_trajectories[:, -10, 0], 850 - resampled_trajectories[:, -10, 1], marker=".", linestyle="-", label="Filtered + interpolated body", color = "blue", ms = 16, linewidth=4)
    ...
    os.chdir(r"C:\Users\Thomas\Documents\stage\edge_consistency_v1\output_files\final\same_yolo_changing_morpho")
    ...
    filtered_tail_data = []
    filtered_body_data = []
    interpolated_head_data = []
    interpolated_tail_data = []
    interpolated_body_data = []

    # Plot all data separately
    plt.figure(figsize=(10, 8))

    # Plot all trajectories first
    for spine_gen in range(37, 193):
        frames, spines = load_frames_spines_from_csv("spines_{}".format(spine_gen))
        
        # Append data to arrays
        head_data.append(spines[:, -1, :])
        tail_data.append(spines[:, 0, :])
        body_data.append(spines[:, -10, :])
        
        filtered_trajectories, filtered_frames = filter_trajectories(spines, frames)
        resampled_trajectories = interpolate_trajectories(filtered_trajectories, filtered_frames, s=5)
        
        # Append filtered and interpolated data to arrays
        filtered_head_data.append(filtered_trajectories[:, -1, :])
        filtered_tail_data.append(filtered_trajectories[:, 0, :])
        filtered_body_data.append(filtered_trajectories[:, -10, :])
        
        interpolated_head_data.append(resampled_trajectories[:, -1, :])
        interpolated_tail_data.append(resampled_trajectories[:, 0, :])
        interpolated_body_data.append(resampled_trajectories[:, -10, :])

    # Plot interpolated trajectories (other colors)
    for data in interpolated_head_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="coral", ms=16, linewidth=4, label="Trace of all heads dif morpho")

    for data in interpolated_tail_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="limegreen", ms=16, linewidth=4, label="Trace of all tail dif morpho")

    for data in interpolated_body_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="royalblue", ms=16, linewidth=4, label="Trace of all body dif morpho")
    ...
    # Plot interpolated trajectories (other colors)
    for data in interpolated_head_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="red", ms=16, linewidth=4, label="Trace of all heads same morpho")

    for data in interpolated_tail_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="green", ms=16, linewidth=4, label="Trace of all tail same morpho")

    for data in interpolated_body_data:
        plt.plot(data[:, 0], 850 - data[:, 1], marker=".", linestyle="-", color="blue", ms=16, linewidth=4, label="Trace of all body same morpho")

    """  # Plot the "good" trajectories on top
    frames, spines = load_frames_spines_from_csv("spines_121")
    filtered_trajectories, filtered_frames = filter_trajectories(spines, frames)
    resampled_trajectories = interpolate_trajectories(filtered_trajectories, filtered_frames, s=5)

    plt.plot(resampled_trajectories[:, -1, 0], 850 - resampled_trajectories[:, -1, 1], marker=".", linestyle="-", color="red", ms=16, linewidth=4, label="Good Head")
    plt.plot(resampled_trajectories[:, 0, 0], 850 - resampled_trajectories[:, 0, 1], marker=".", linestyle="-", color="green", ms=16, linewidth=4, label="Good Tail")
    plt.plot(resampled_trajectories[:, -10, 0], 850 - resampled_trajectories[:, -10, 1], marker=".", linestyle="-", color="blue", ms=16, linewidth=4, label="Good Body") """

    # Set the legend to avoid duplicate labels
    handles, labels = plt.gca().get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    plt.legend(by_label.values(), by_label.keys(), fontsize=16)

    plt.savefig("trajectories_final_compa{}.png".format(0))
    plt.show()
    ...
    plt.plot(spines[:, -1, 0], 850 - spines[:, -1, 1], marker=".", linestyle="--", label="Head", color = "coral", ms = 16, linewidth=4)
    plt.plot(spines[:, 0, 0], 850 - spines[:, 0, 1], marker=".", linestyle="--", label = "Tail", color = "limegreen", ms = 16, linewidth=4)
    plt.plot(spines[:, -10, 0], 850 - spines[:, -10, 1], marker=".", linestyle="--", label="Body", color = "royalblue", ms = 16, linewidth=2)
    ...
    dist_spine_skin = np.load("ref_width380.npy")
