...
|       ├── filtered_spine{}.npy        # filtered spine estimation for a given frame
|       ├── filtered_spine{}.png        # filtered spine estimation for a given frame
|       ├── corrected_spine{}.npy       # corrected spine estimation for a given frame
|       ├── corrected_spine{}.png       # corrected spine estimation for a given frame
|       ├── applied_shape{}.npy         # applied shape for a given frame
|       └── applied_shape{}.png         # applied shape for a given frame
|        
...
end_ToI = 2*60 + 45 #1*60 + 56#13*60+24 # 14*60 + 2 #
...
def delete_files_except_patterns(directory):
    ...
    ref_width = re.compile(r'ref_width\d+\.npy')
    ref_length = re.compile(r'ref_length\d+\.npy')
    ...
    for filename in os.listdir(directory):
        ...
        #if not (yolo_edge_pattern.match(filename) or faulty_polygon_pattern.match(filename) or ref_length.match(filename) or ref_width.match(filename)):
        if not (ref_length.match(filename) or ref_width.match(filename)):
            try:
                # Delete the file
                os.remove(file_path)
...
if __name__ == '__main__':
    ...
    for test in range(0, 1):
        ...
        fourth_pipeline_step("spines_{}".format(test), spines) # Save data
        os.chdir(temporary_files_path) # Go back to temporary folder 
        
        # =====================
        # Fifth step - Data filter and interpolation
        os.chdir(final_output_path) # Go to final folder to save data
        frames, spines = load_frames_spines_from_csv("spines_{}".format(test))
        fifth_pipeline_step(frames, spines, "spines_interpolated_{}".format(test))
        
        # =====================
        # Sixth step - Generate csv for lilypad
        os.chdir(final_output_path)
        sixth_pipeline_step("spines_interpolated_{}".format(test), shape_width)
